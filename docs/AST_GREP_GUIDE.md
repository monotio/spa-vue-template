...
| Rule | Enforces |
| --- | --- |
| `cs-no-direct-utcnow` | inject `TimeProvider`, never `DateTime.UtcNow` |
...
set to avoid dead rules): no sync-over-async (`.Result`, `.Wait()`), no
`SaveChanges()` (sync) once EF lands, ProblemDetails-shape enforcement in
